...
exports['jst'] = {
  main: function(test) {
    ...
    test.equal(expect, result, "should wrap the template with define for CommonJS pattern");

    expect = grunt.file.read("test/expected/commonjs_wrapper_no_ns.js");
    result = grunt.file.read("tmp/commonjs_wrapper_no_ns.js");
    test.equal(expect, result, "should wrap the template with define for CommonJS pattern and return the function itself with no namespace");
